...
- **The grader is account-aware, because personal figures never live in the KB.** "How much is left to pay on my Murabaha contract?" with an account attached dead-ended at `no_answer` on the live deployment: a passages-only grader is *correct* to fail it — no KB passage holds this customer's balance — and `reformulate` cannot rewrite its way to one, so both retries burned while `generate`, which renders the record and could answer, sat unreached one node downstream. The route was right; the grader was the only account-blind node on the path. The grade prompt carries the record's outline (products and field names, never values — availability is the verdict's business, the figures are `generate`'s), and personal questions are graded on product coverage. With no account attached, product-covering passages still pass so `generate` can say it cannot see the account — an instruction its prompt always carried, unreachable while the grader dead-ended those turns first. The home-mortgage negative above is pinned *with* a record attached: an outline must not rubber-stamp a product Mal does not offer, and no record field covers a mortgage either. That pin failed live twice before holding, for two separate reasons: the first account-aware phrasing let the outline sway availability questions (fixed with an explicit "only facts about existing holdings" carve-out in the grade prompt), and then the wakala doc's Related Mal Documents blurb described Ijara as "lease-based financing for property and vehicles" — making "Mal finances property" a *grounded, citable* claim while every other document says auto-only. The corpus was made self-consistent (the blurb now reads vehicle-only, matching the Ijara doc itself) rather than the grader tightened further: a cross-reference blurb that over-describes a product is a retrieval landmine no prompt can durably defuse.
